...
{
    public GameObject lampController;
    public LampControl lampControl;
    ...
    private void Start()
    {
        lampController = transform.Find("Camera").gameObject;
        lampControl = lampController.GetComponent<LampControl>();
    ...
    void DetectEnemy()
    {
        ...
        {
            Vector3 lampDirection = Vector3.Scale(lampController.transform.forward, new Vector3(1, 0, 1));
            Vector3 targetDirection = Vector3.Scale((hitColliders[i].transform.position - lampController.transform.position), new Vector3(1, 0, 1)).normalized;

            float deltaAngle = Vector3.Angle(targetDirection, lampDirection);
            // Debug.Log(deltaAngle);

            if (deltaAngle > lampControl.lampLight.spotAngle / 3)
            {
                continue;
            }
    ...
    private void OnDrawGizmos()
    {
        if (showGizmos)
        {
            ...
            Quaternion lampRotation = Quaternion.Euler(0, lampController.transform.eulerAngles.y, 0);
